...
def generate_bones_backup(num_bones_x, num_bones, bound, device):
    """
...
def generate_bones(num_bones_x, num_bones, bound):
    """
    num_bones_x: bones along one direction
    bones: x**3,9
    """
    center =  torch.linspace(-bound, bound, num_bones_x)
    center =torch.meshgrid(center, center, center)
    center = torch.stack(center,0).permute(1,2,3,0).reshape(-1,3)
    center = center[:num_bones]
